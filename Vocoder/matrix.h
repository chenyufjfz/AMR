...
public:
   ...
   matrixT& operator = (const vector<T>& v) _NO_THROW;
   ...
   // Row, column vectors
   vector<T> Row(size_t r) const _THROW_MATRIX_ERROR;
   vector<T> Col(size_t c) const _THROW_MATRIX_ERROR;
...
MAT_TEMPLATE inline
matrixT::Matrix (const vector<T>& v)
{
	_m = new base_mat( v.size(), 1, 0);
...
// Row, column vectors
MAT_TEMPLATE inline vector<T>
matrixT::Row(size_t r) const _THROW_MATRIX_ERROR
{
	vector<T> ret;

	if (r>=RowNo())
		REPORT_ERROR("matrixT::Row(): Index out of range!");
	ret.resize(ColNo());
...
MAT_TEMPLATE inline vector<T>
matrixT::Col(size_t c) const _THROW_MATRIX_ERROR
{
	vector<T> ret;
